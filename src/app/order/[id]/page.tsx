...
import { Barricade, Lock, TrashSimple } from '@phosphor-icons/react/dist/ssr';
...
const OrderPage = () => {
    ...
    const { userInfo, setAuthModal } = useStore()
    const authenticating = false;


    const initOrder = async () => {
        const response: any = await userPrivateService.getOneOrder(orderID);
                                                            ...
                                                            <div className="flex gap-2 flex-wrap mb-2">
                                                                {
                                                                    Array(3).map((attribute, idx) => (
                                                                        <div
                                                                            key={index}
                                                                            className="border border-gray-500 px-2 rounded-lg bg-gray-200 animate-pulse rounded-lg w-24"
                                                                        >
                                                                            &nbsp;
                                                                        </div>
                                                                    ))
                                                        ...
                                                        <div className="grow">
                                                            ...
                                                            {item.variant &&
                                                                <div className="flex gap-2 flex-wrap">
                                                                    {item.variant && item.product.variants && item.product.variants.find((v: any) => v._id === item.variant) ?

                                                                        <>
                                                                            {
                                                                                Object.entries(item.product.variants.find((v) => v._id === item.variant).attributes).map(([key, attribute]) => (
                                                                                    <div
                                                                                        key={key}
